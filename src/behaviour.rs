...
impl Bitswap {
    ...
    /// Broadcast unrequested blocks to all peers
    pub fn broadcast_blocks(&mut self, blocks: BTreeSet<Block>) {
        if blocks.is_empty() {
            return;
        }
        let peers = self.connected_peers.keys().cloned().collect();
        debug!("bitswap: broadcast_blocks");
        for (peer_id, blocks) in zip_with_clones(peers, blocks) {
            debug!("  queueing block for peer {}", peer_id.to_base58());
            self.send_messages(peer_id, Message::want_response(blocks.into_iter()));
        }
    }
